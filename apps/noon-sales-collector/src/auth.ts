...
export async function loginNoon(credential: NoonCredential, fetchImpl: typeof fetch = fetch): Promise<NoonSession> {
  const response = await fetchImpl(LOGIN_URL, {
    method: "POST",
    headers: { "User-Agent": "NoonSalesCollector/1.0", "Content-Type": "application/json" },
