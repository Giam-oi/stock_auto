...
export async function loadCredential(filePath: string, expectedStore: StoreConfig): Promise<NoonCredential> {
  ...
  try {
    ...
    throw new Error(`Invalid credential ${basename(filePath)}: file is not readable JSON`);
  }
  ...
  if (!/^-----BEGIN (?:RSA )?PRIVATE KEY-----[\s\S]+-----END (?:RSA )?PRIVATE KEY-----\s*$/.test(privateKey)) {
    throw new Error(`Invalid credential ${basename(filePath)}: private_key must be a PEM private key`);
  }
  if (projectCode !== expectedStore.projectCode) {
    throw new Error(`Credential ${basename(filePath)} project_code mismatch`);
  }
  return { type: "apijwt", key_id: keyId, private_key: privateKey, project_code: projectCode };
}
